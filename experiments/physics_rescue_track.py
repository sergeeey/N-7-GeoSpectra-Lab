...
print(f"  KILL: Witten-Lichnerowicz for R>0" if h1_killed else "  NOTE: non-zero")
results["H1_gauge_bundle"] = {
    "a_hat_S3": a_s3, "a_hat_S6": a_s6, "a_hat_product": a_product,
...
h2_s6 = 0
print(f"  H^2(S^6) = {h2_s6}")
h2_killed = (h2_s6 == 0)
print(f"  KILL: Freund-Rubin requires H^2 != 0" if h2_killed else "  Flux possible")
results["H2_flux"] = {
    "H2_S6": h2_s6,
    "kill_reason": "H^2(S^6)=0 -> no harmonic 2-forms -> flux ansatz impossible",
    "killed": h2_killed
}
...
print(f"  KILL: N_gen ~ |chi|/|Gamma| = 0" if h3_killed else "  Generation formula applies")
results["H3_orbifold"] = {
    "chi_S3": chi_s3, "chi_S6": chi_s6, "chi_product": chi_product,
...
results["H4_ncg"] = {
    "KO_dim_S3": ko_s3, "KO_dim_S6": ko_s6, "KO_dim_product": ko_product,
