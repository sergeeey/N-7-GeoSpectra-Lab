...
def get_fp(L, k=15):
    ...
    return {"r": r, "d": float(deff), "cv": cv, "ds": dens.tolist() if dens is not None else [], "ev": ev.tolist()}
...
def phase_cell(W, n, pair, laps):
    ...
    if len(fe2s) >= 3:
        ...
        for i in range(len(fe2s)):
            test_x = np.array([fe2s[i]])
            test_y = lbs[i]
            pred = 1 if test_x[0][0] > 4 else 0
            if pred == test_y:
                correct += 1
        two_acc = correct / len(fe2s)

    p = "erased"
    if sd_acc >= 0.95 or (two_acc and two_acc >= 0.95):
        p = "recoverable"
    elif sd_acc >= 0.70 or (two_acc and two_acc >= 0.70):
        p = "degraded"

    return {"W": W, "N": n, "pair": f"{g1}_vs_{g2}",
            "sd_acc": sd_acc, "two_acc": two_acc, "mean_sd": mean_sd,
            "phase": p, "n_samples": len(sd_vals)}

# ── MAIN ────────────────────────────────────────────────────────
print("="*60)
print("PHASE 4B: Spectral Recoverability Phase Diagram")
print(f"Grid: {len(W_VALUES)} W x {len(N_CURVED)} N x {len(PAIRS)} pairs = {len(W_VALUES)*len(N_CURVED)*len(PAIRS)} cells")
print("="*60)
...
for n in N_CURVED:
    ...
    laps = build_laps(n)
    print(f"  T4: {laps['T4'].shape[0]}, S3xS1: {laps['S3xS1'].shape[0]}, S2xS2: {laps['S2xS2'].shape[0]}")
    ...
    for pair in PAIRS:
        for W in W_VALUES:
            ...
            if cell:
                ...
                two_str = f"2f={cell['two_acc']:.1%}" if cell['two_acc'] else "2f=N/A"
                print(f"  [{cell_idx}/{total}] W={W:2d} {pair[0]}v{pair[1]:5s}: sd={cell['sd_acc']:.1%} {two_str:8s} -> {sym} {cell['phase']}")
            else:
