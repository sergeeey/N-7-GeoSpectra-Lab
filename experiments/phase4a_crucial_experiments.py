    ...
    for s in seed_list:
        for W in W_list:
            for n1,n2,lb in PAIRS:
                ...
                if p1 and p2: data.append((fe(p1,p2),lb,fe2(p1,p2),lb))
    return data

print("="*60); print("CRUCIAL EXPERIMENTS: Saving W=20"); print("="*60)
results = {}

# ==== EXP 1: H2 — Train on W∈[0,10], test W=20 ====
print("\n[EXP 1] H2: Overfitting — train W≤10, test W=20")
train1 = collect([0,1,5,10], SEEDS_TRAIN, K)
X1 = np.array([d[0] for d in train1]); y1 = np.array([d[1] for d in train1])
test1 = collect([20], SEEDS_TEST, K)
X1t = np.array([d[0] for d in test1]); y1t = np.array([d[1] for d in test1])
...
results["H2_overfitting"] = {"train_W": [0,1,5,10], "test_W": 20, "acc": acc1, "verdict": "H2 killed" if acc1>0.7 else "H2 confirmed"}

# ==== EXP 2: H4 — Only top 2 features ====
print("\n[EXP 2] H4: Feature selection — only sd_dist + d_eff_spread")
train2 = collect([0], SEEDS_TRAIN+SEEDS_TEST, K)
X2 = np.array([d[2] for d in train2]); y2 = np.array([d[3] for d in train2])
train_n = len(SEEDS_TRAIN) * len([0]) * len(PAIRS)
clf2 = RandomForestClassifier(n_estimators=30,max_depth=4,random_state=42)
clf2.fit(X2[:train_n], y2[:train_n])
acc2 = float(clf2.score(X2[train_n:], y2[train_n:]))
print(f"  2 features (sd_dist, d_eff_spread)")
print(f"  Test: {acc2:.1%} {'→ H4 confirmed' if acc2>0.7 else '→ H4 killed'}")
results["H4_features"] = {"n_features": 2, "acc": acc2, "verdict": "H4 confirmed" if acc2>0.7 else "H4 killed"}
...
if test3:
    X3 = np.array([d[0] for d in train3]); y3 = np.array([d[1] for d in train3])
    X3t = np.array([d[0] for d in test3]); y3t = np.array([d[1] for d in test3])
    ...
    results["H1_k30"] = {"k": 30, "acc": acc3, "baseline_k15": 0.625}
else:
    print(f"  k=30 FAILED (eigsh convergence)")
    ...
    for i,s in enumerate(seed_list):
        for W in W_list:
            for n1,n2,lb in PAIRS:
                s1 = s
                s2 = s + 1000
                L1=ad(laps[n1],W,s1) if W>0 else laps[n1]
                L2=ad(laps[n2],W,s2) if W>0 else laps[n2]
                p1=get_fp(L1,n1,k); p2=get_fp(L2,n2,k)
...
if test4:
    ...
    print(f"  → {verdict}")
    results["H6_diffseeds"] = {"threshold_acc_same_seed": 0.909, "threshold_acc_diff_seeds": float(sd_correct), "verdict": verdict}
else:
...
print(f"  H4 (2 features):      sd+d_eff only = {results['H4_features']['acc']:.1%} — {results['H4_features']['verdict']}")
if results['H1_k30']['acc']:
    print(f"  H1 (k=30):            k=30 at W=20 = {results['H1_k30']['acc']:.1%} (vs 62.5% k=15)")
else:
    print(f"  H1 (k=30):            FAILED")
if results['H6_diffseeds'].get('threshold_acc_diff_seeds'):
    print(f"  H6 (diff seeds):      threshold {results['H6_diffseeds']['threshold_acc_diff_seeds']:.1%} — {results['H6_diffseeds']['verdict']}")
