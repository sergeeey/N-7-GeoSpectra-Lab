...
Xs = None
if same_test:
    Xs = np.array([x[0] for x in same_test])
    ys = np.array([x[1] for x in same_test])
...
# T3: Curved boundary
print("[T3] Curved boundary (ML):")
boundary = []
for W in [0, 5, 10, 15, 20]:
    d = collect([("S3xS1", "S2xS2", 1)], [W], SEEDS[:5], K)
    if d:
        Xw = np.array([x[0] for x in d])
        yw = np.array([x[1] for x in d])
        acc = clf.score(Xw, yw)
...
res["t3_curved_boundary"] = {
    "by_W": {str(W): a for W, a in boundary},
    "degrades": degrades,
    "v": "PASS" if degrades else "FAIL",
}
...
clf_ab.fit(X_ab[:n], y[:n])
a_ab = clf_ab.score(X_ab[n:], y[n:])
a_base = clf.score(X[n:], y[n:])
drop = a_base - a_ab
print(
    f"[T4] Ablation: full={a_base:.1%} no-sd={a_ab:.1%} drop={drop:.1%} {'PASS' if drop > 0.1 else 'FAIL'}"
)
res["t4_ablation"] = {
    "full": float(a_base),
    "no_sd": float(a_ab),
    "drop": float(drop),
    "v": "PASS" if drop > 0.1 else "FAIL",
}
