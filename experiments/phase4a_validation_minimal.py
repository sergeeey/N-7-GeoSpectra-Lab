...
def get_fp(L, n, k=8):
    ...
    k = min(k, n0 - 2)
    try:
        ev = eigsh(L, k=k, which="SM", return_eigenvectors=False, tol=1e-7)
...
print("=" * 50)
print("VALIDATION SUITE (minimal)")
print("=" * 50)
...
for s in ST:
    for a, b in diff:
        trc.append((0, s, a, b, 1))
    for a, b in same:
        trc.append((0, s, a, b, 0))
trd = cl(trc)
...
for s in SE:
    for W in WT:
        for a, b in diff + same:
            tec.append((W, s, a, b, 1 if a != b else 0))
ted = cl(tec)
...
for s in ST + SE:
    for W in [0, 5]:
        c1.append((W, s, "T4", "T4", 0))
d1 = cl(c1)
if d1:
    X1 = np.array([d[0] for d in d1])
    y1 = np.array([d[1] for d in d1])
    n_split = len(X1) // 2
    cp = RandomForestClassifier(n_estimators=30, max_depth=5, random_state=42)
    cp.fit(X1[:n_split], y1[:n_split])
    p = cp.predict(X1[n_split:])
    a = np.mean(p == 0)
...
trc2 = []
for s in ST:
    for a, b in [("T4", "S1"), ("T4", "S2"), ("S1", "S2")]:
        trc2.append((0, s, a, b, 1))
    for a, b in [("T4", "T4"), ("S1", "S1"), ("S2", "S2")]:
        trc2.append((0, s, a, b, 0))
trd2 = cl(trc2)
...
for s in SE:
    for W in [0, 5]:
        for a, b in [("T4", "S3"), ("S1", "S3"), ("S2", "S3")]:
            tec2.append((W, s, a, b, 1))
ted2 = cl(tec2)
if ted2:
    ...
    clf2.fit(Xtr2, ytr2)
    a_u = float(clf2.score(Xte2, yte2))
    print(f"[3] UNSEEN S3xS2: acc={a_u:.1%} {'PASS' if a_u > 0.6 else 'FAIL'}")
    results["unseen_geometry"] = {"unseen_acc": a_u}

# Unseen W — clf trained on W=0 (trc), Xte built with WT=[5,20]
print(f"[4] UNSEEN W: train W=0, test W={WT}: acc={acc2:.1%} {'PASS' if acc2 > 0.6 else 'FAIL'}")
results["unseen_W"] = {"train_W": 0, "test_W": WT, "acc": acc2}

# Permutation
ps = []
for i in range(5):
    yp = np.random.default_rng(i).permutation(ytr)
    cp = RandomForestClassifier(n_estimators=30, max_depth=5, random_state=i)
    cp.fit(Xtr, yp)
    ps.append(cp.score(Xte, yte))
mp, sp = np.mean(ps), np.std(ps)
print(
    f"[5] PERMUTATION: {mp:.1%}±{sp:.1%} vs actual {acc2:.1%} {'PASS' if acc2 > mp + 2 * sp else 'FAIL'}"
)
...
for t in np.linspace(0, 30, 31):
    ...
    if a > ba:
        ba, bt = a, float(t)
print(
    f"[6] THRESHOLD: sd>{bt:.0f} -> {ba:.1%}, ML={acc2:.1%} {'PASS (ML wins)' if acc2 > ba else 'NOTE'}"
)
results["threshold_vs_ml"] = {"thresh_acc": float(ba), "ml_acc": float(acc2), "best_thresh": bt}

# Templates — verify no exact feature overlap between train and test sets
tp = set(f"{a}_vs_{b}" for a, b in diff + same)
Xtr_set = set(map(tuple, np.round(Xtr, 8).tolist()))
overlap = sum(1 for row in Xte if tuple(np.round(row, 8).tolist()) in Xtr_set)
passed_7 = overlap == 0
print(
    f"[7] TEMPLATES: {len(tp)} pair types; exact duplicates={overlap}/{len(Xte)} {'PASS' if passed_7 else 'FAIL'}"
)
results["template_leakage"] = {
    "n_pair_types": len(tp),
    "exact_duplicates": overlap,
    "v": "PASS" if passed_7 else "FAIL",
}
