...
def _collect_audit_files() -> list[Path]:
    targets: list[Path] = [
        REPO_ROOT / "README.md",
        REPO_ROOT / "PROJECT_CURRENT_STATE.md",
    ]
...
FORBIDDEN: list[tuple[str, str]] = [
    ...
    (r"\bS.3.{0,5}S.1\b.*\bsolved\b",
     "S³×S¹ solved claim"),
    (r"\blambda\s+(?:is\s+)?fixed\b",
     "lambda fixed claim (λ = FREE_COUPLING_PARAMETER always)"),
    (r"\bλ\s+(?:is\s+)?fixed\b",
     "λ fixed claim (unicode form)"),
    (r"\btom(?:'s)?\s+ansatz\b.*\bsolved\b",
     "Tom ansatz solved claim"),
    (r"\bH-T1\s+confirmed\b",
     "H-T1 confirmed claim"),
    (r"\beq\.?\s*49\b.*\b(?:derived|solved|confirmed)\b",
     "eq.49 derived/solved/confirmed claim"),
    (r"\btrue\s+geometry\s+is\b",
     "true geometry claim"),
    (r"\bφ.?1[,_]?1\b.*\bidentified\s+as\s+full\s+mode\b",
     "φ₁₁ full mode identification claim"),
...
NEGATION_RE = re.compile(
    r"(?:"
    r"\b(?:not?|no|never|blocked|forbidden|does\s+not|overclaim|removed|absent|free)\b"
...
def test_no_overclaim_phrases(audit_path: Path) -> None:
    ...
    if violations:
        ...
        pytest.fail(
            f"Overclaim phrase(s) in {audit_path.relative_to(REPO_ROOT)}:\n{lines}"
        )
