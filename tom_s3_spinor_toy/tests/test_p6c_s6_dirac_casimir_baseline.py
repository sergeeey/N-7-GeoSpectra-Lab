"""Smoke tests for the S6 Dirac/Casimir baseline contract."""

from __future__ import annotations

from s6_g2_su3_dirac_casimir_baseline import (
    build_s6_dirac_casimir_baseline,
    s6_dirac_casimir_baseline_summary,
)


def test_s6_dirac_casimir_baseline_preserves_the_geometric_fence() -> None:
    ...
    assert "baseline only" in baseline.scope


def test_s6_dirac_casimir_baseline_keeps_the_claim_fence_explicit() -> None:
    """The baseline layer must not drift into forbidden claims."""

    summary = s6_dirac_casimir_baseline_summary()
    ...
    assert summary["scope"] == "S6 Dirac/Casimir baseline only; no spectrum or gauge claim"
