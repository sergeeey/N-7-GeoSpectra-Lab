...
Lock down the S6 spectrum operator fence after stabilization, without
computing a spectrum.

## Implemented

[CODE] Added the operator-lockdown contract:

```text
s6_g2_su3_spectrum_operator_lockdown.py
tests/test_p6h_s6_spectrum_operator_lockdown.py
```

[CODE] The lockdown layer keeps the current S6 contract fixed as:
...
[CODE] The lockdown layer retains the spectrum target:
...
[INFERRED] This is an operator lockdown contract only. It does not compute a
...
P6H_S6_SPECTRUM_OPERATOR_LOCKDOWN = started
