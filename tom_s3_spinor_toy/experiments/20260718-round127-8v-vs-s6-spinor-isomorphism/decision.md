...
| Once `Φ` and `S` are found, redo round126's `B-L` comparison literally | **[DONE, round128, 2026-07-19]** Transported round124's centralizer through the verified `S` and compared directly against `G15.BmL`. Result: `NO_LITERAL_MATCH`, checked exhaustively across all 12 members of `Aut(su(3))` (not just one `S`) — relative residuals `{0.53,...,1.00}`, zero clean matches — the abstract isomorphism is real, but this specific centralizer direction is not `B-L` under any valid choice of intertwiner. |
