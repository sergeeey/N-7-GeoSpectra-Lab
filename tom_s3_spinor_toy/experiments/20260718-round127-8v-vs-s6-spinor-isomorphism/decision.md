...
|---|---|
| Find the Cartan-Weyl alignment `Φ` between `su3_v` and `su3_sigma` | Identify each copy's 2-dim Cartan subalgebra and root vectors independently (via simultaneous diagonalization / generalized eigenvalue problem), then match roots by their eigenvalue pattern — standard but nontrivial linear algebra, not attempted here |
| Once `Φ` and `S` are found, redo round126's `B-L` comparison literally | Transport round124's centralizer through the corrected `S` and compare directly against `G15.BmL` as matrices, not ratios — this is the actual payoff of resolving this round's gap |
