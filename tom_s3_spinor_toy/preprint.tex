...
A geometric obstruction (Theorem~\ref{thm:yukawa-deg}) proves that all
inter-generation Yukawa couplings are equal on $S^6$; mass hierarchy requires
...
compactifications that simultaneously fix $N_{\mathrm{gen}} = 3$ and the SM fermion
quantum numbers without free parameters.
...
$\mathrm{Spin}(8)$-level input (experiment E-L3B proves no $G_2$-equivariant invariant
can distinguish the three channels; see \S\ref{sec:open}) and is the remaining open
component of L3.
...
kernel count (L4B). We prove L4B directly via the Kostant--Parthasarathy formula,
bypassing L4A. L4A remains an open problem but is no longer required for the main conclusion.
...
\subsection{\texorpdfstring{$G_2$}{G2}-equivariance and Kostant--Parthasarathy formula (L4B --- proved)}
\label{sec:schur}
...
\textbf{Trivial $G_2$-representation $\mathbf{1}$.}
...
\]
Since $\ind(D^+_{S^-})=1$ and this is the only component that can contribute zero modes,
the restricted operator has index $+1$, hence rank $1$ and $\dim\ker(D^+|_{\mathbf{1}}) = 1$.
...
$\mathrm{rank}(D^-) = \mathrm{rank}(D^+) = 1$,
giving $\dim\ker(D^-|_{\mathbf{1}}) = 1 - 1 = 0$.
Therefore $\dim\mathrm{coker}(D^+_{S^-}) = 0$. \quad\textbf{(L4B, proved)}

\begin{corollary}[Exact kernel]
Combining $\ind(D^+_{S^-})=1$ (Lemmas L1--L2) with L4B (proved above):
\begin{align}
  \dim\ker(D^+) - \dim\mathrm{coker}(D^+) &= 1 \quad\text{(index, proved)},\\
  \dim\mathrm{coker}(D^+) = \dim\ker(D^-) &= 0 \quad\text{(L4B, proved)},\\
  \Rightarrow\;\; \dim\ker(D^+) &= 1,\quad \dim\ker(D^-)=0. \quad\textbf{(proved)}
\end{align}
The kernel of the twisted Dirac operator $D^+_{S^-}$ is exactly one-dimensional.
Combined with L3 (three independent triality channels, open problem), this gives
$N_{\mathrm{gen}}=3\times 1=3$.
\end{corollary}
...
The Kostant--Parthasarathy formula (L4B, this work) establishes $\dim\ker(D^+_{S^-})=1$
and $\dim\mathrm{coker}(D^+_{S^-})=0$ (proved). $\ind=1$ for each of the three SO(8)
...
Experiment E-L3B (this work) proves the $G_2$-equivariant bundles are
pairwise isomorphic, so the distinction requires $\mathrm{Spin}(8)$ fibre symmetry input.
...
\begin{proposition}[Analytic radius ratio, G66]\label{prop:kappa}
  ...
  This is a zero-parameter result that depends only on the dimension of $S^6$;
...
\end{theorem}
  ...
  Experiment E-L3B (this work) proves this obstruction theorem: by the homogeneous bundle
  correspondence, $G_2$-equivariant vector bundles on $G_2/\mathrm{SU}(3)$ are classified
  by $\mathrm{SU}(3)$-representations. Since all three channels restrict to the
  \emph{same} $\mathrm{SU}(3)$-module $(1,0)\oplus(0,1)\oplus 2\cdot(0,0)$, the bundles
  $E_v\cong E_s\cong E_c$ are pairwise isomorphic as $G_2$-equivariant bundles. No
  $G_2$-invariant, including nearly-K\"ahler instanton connections, can distinguish them.
  ...
  follows. Confirming this physical input is the remaining open question, to be addressed in
  collaboration with T.~Lawrence.
...
\item \textbf{L4B --- $G_2$-equivariant character computation (\emph{resolved}).}
  Proved in \S\ref{sec:schur} via the Kostant--Parthasarathy formula: the positive spectral
  gap on all non-trivial $G_2$-isotypic components of $\Gamma(S^+\otimes S^-)$, combined
  with the trivial-component index argument, gives $\dim\ker(D^+_{S^-})=1$ and
  $\dim\mathrm{coker}(D^+_{S^-})=0$.
  The representation-theoretic computation builds on~\cite{AgrHofLawn2023,Agricola2002}.
...
\begin{theorem}[Yukawa Degeneracy]\label{thm:yukawa-deg}
  Let $\psi^{(\alpha)}$ denote the unique positive-chirality zero mode of $D^{(\alpha)}$
...
\begin{proof}
  By the Lichnerowicz gap (Lemma~L4, \S\ref{sec:lichnerowicz}), each $\ker(D^{(\alpha)}|_{\Sigma^+})$
  ...
  By the E-L3B bundle isomorphism theorem (\S\ref{sec:index}), $E_v\cong E_s\cong E_c$
...
\end{proof}
  ...
  Consequently, Yukawa mass hierarchy cannot arise from $S^6$ geometry alone; it must originate
  from the NCG finite Dirac operator $D_F$ with its four free Yukawa matrices
  $\{Y_\nu, Y_e, Y_u, Y_d\}$ \cite{CCM2006}.
  The obstruction is specific to $S^6$: the role of $S^3$ Kaluza-Klein coupled-mode mixing
  in lifting inter-generation Yukawa degeneracy in the full 9-dimensional analysis remains
  an open question.
