...
Therefore $\dim\mathrm{coker}(D^+_{S^-}) = 0$. \tag{L4B, proved}
...
and $\dim\mathrm{coker}(D^+_{S^-})=0$ per channel (proved).
The conjecture $N_{\mathrm{gen}}=3$ requires the one remaining open problem:
(i)~three independent triality channels with $\ind=1$ each (L3, open; \S\ref{sec:open}).
