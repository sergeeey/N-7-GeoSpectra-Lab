...
\item \textbf{Physical origin of the twisted operator $D\otimes S^-$ [open --- no parent
  action identified].} The entire three-generation count follows from twisting the
  internal Dirac operator by $S^-$ specifically (\S\ref{sec:ngen}); Proposition~T2
  (untwisted Dirac on $S^a\times S^b$ has strictly positive minimum eigenvalue, i.e.\
  index $0$) shows this twist choice does all of the work. No higher-dimensional parent
...
\item \textbf{Fermion mass hierarchy.} The index theorem gives $N_{\mathrm{gen}}=3$ with equal
  ...
  structure --- so no such cross-term can exist.
...
derivations (implemented as a Python/\texttt{sympy} test suite, 2488 tests:
2484 passing, 4 skipped),
