...
gap on every \emph{non-trivial} $G_2$-isotypic component, ruling out zero modes there
(this part does not depend on L4A). On the remaining $G_2$-trivial component, however,
...
every non-trivial $G_2$-isotypic component (proved); on the remaining trivial component, it
...
\item \textbf{L4B --- $G_2$-equivariant character computation (\emph{partially resolved; rank
  on the trivial component open}).}
  The Kostant--Parthasarathy formula (\S\ref{sec:schur}) proves a positive spectral gap on
  all non-trivial $G_2$-isotypic components of $\Gamma(S^+\otimes S^-)$ --- this part is
  proved. On the remaining trivial component, the map $D^+|_{\mathbf 1}:\CC^2\to\CC^1$ has
  index $1$ regardless of its rank (index of any linear map between spaces of these
  dimensions is automatically $2-1=1$), so the global index alone does not fix
  $\dim\ker(D^+_{S^-})\in\{1,2\}$. We assume $\dim\ker(D^+_{S^-})=1$
  (equivalently rank$=1$) as a working hypothesis, not a proved fact; ruling out rank$=0$
  requires an explicit torsion-term (Clifford multiplication by the $G_2$-invariant
  associative $3$-form) computation distinguishing the Levi-Civita from the cubic Dirac
  operator on this naturally reductive (non-symmetric) coset, since the naive
  per-summand Casimir formula is only a theorem in the symmetric/cubic-Dirac case.
  See experiment \texttt{20260708-dolan-casimir-g2su3} (parked; explicit computation
  needed). The representation-theoretic setup builds on~\cite{AgrHofLawn2023,Agricola2002}.
