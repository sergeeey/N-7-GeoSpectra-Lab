...
\begin{itemize}
  ...
  \item $D_F$ --- the finite Dirac operator with Yukawa structure; $J_F^2=+1$,
        $\{J_F,\gamma_F\}=0$, $[D_F,J_F]=0$. These three relations are verified
        explicitly; the identification of the resulting sign triple with
        KO-dimension~6 follows CCM~\cite{CCM2006} and is not independently
        derived here.
...
\item \textbf{L3b --- Channel independence (\emph{open; all internal geometric avenues
  ...
  physical input external to the present geometric construction.

  \textbf{Partial algebraic advance (internal verification, this work).} A subsequent
  search for a distinguishing structure outside the pure $\mathfrak{so}(8)$-internal
  centralizer route found one candidate: splitting the octonion fibre
  $\mathbb{O}=\mathbb{R}^8$ into two quaternionic blocks, $H:=\mathrm{span}(e_0,\dots,e_3)$
  and $H\ell$ (its Cayley--Dickson-doubling complement, $\ell$ the doubling generator),
  and taking the full $\mathrm{SO}(4)\times\mathrm{SO}(4)$ (not restricted to octonion
  automorphisms) genuinely distinguishes all three representations
  $\mathbf{8}_v,\mathbf{8}_s,\mathbf{8}_c$
  via block-chirality operators $\Gamma_A,\Gamma_B$ (same-block chirality $\Gamma_A=\Gamma_B$
  on $\mathbf{8}_s$, opposite-block chirality $\Gamma_A=-\Gamma_B$ on $\mathbf{8}_c$) --- the
  first candidate in this investigation to distinguish all three channels, not merely
  $\mathbf{8}_v$ from $\{\mathbf{8}_s,\mathbf{8}_c\}$. This escapes the obstruction that
  closed every earlier candidate: $\mathrm{SO}(4)\times\mathrm{SO}(4)$ has rank 4, and no
  rank-4 group embeds in the rank-3 $\mathrm{SO}(7)$ that every $G_2$- or
  $\mathrm{SO}(6)$-subgroup route was confined to. The subalgebra is itself verified
  triality-invariant by explicit construction. Applying the same PASS/PARTIAL/NO
  criteria used throughout this analysis to this candidate gives: the algebraic
  distinguishability condition is satisfied, but the further conditions are not
  established, for two separate reasons, not one. First, \emph{physical identification}:
  nothing in the construction fixes which (if either) $\mathrm{SO}(4)$ factor
  corresponds to $S^3$'s actual $\mathrm{SU}(2)_L\times\mathrm{SU}(2)_R$ gauge fields,
  and what the other factor would represent physically is entirely unaddressed. Second,
  \emph{dynamical consistency}: the current proof of the exact kernel dimension
  ($\dim\ker=1$, not merely $\geq 1$; \S\ref{sec:kernel}, gate G74A's Lemma B) rests on
  Schur's lemma applied to the \emph{exact} $G_2$ symmetry of the geometry --- a proof
  technique that does not merely require re-checking once $G_2$ is broken by this
  construction, but structurally no longer applies at any nonzero such breaking, and no
  $\mathrm{Spin}(4)\times\mathrm{Spin}(4)$-equivariant analogue of this argument exists
  with current tools (this candidate symmetry acts only on the octonion fibre, not on
  the base manifold, so the base-transitivity argument the original proof relies on has
  no analogue here). These are the same open $\mathrm{Spin}(8)$-fibre-symmetry-type
  physical input named above, now with one concrete algebraic candidate on the table
  rather than none, and with its own specific obstructions precisely located.
