...
%% ---- Abstract ----
\begin{abstract}
We construct a candidate mechanism for the Standard Model gauge structure and
fermion quantum numbers of one generation from the geometry of $S^3\times S^6$,
built from an exact index computation on the $S^6$ factor; whether this mechanism
also yields a genuine massless 4D zero mode of the \emph{full} nine-dimensional
internal Dirac operator on $S^3\times S^6$ --- as required under standard
Kaluza--Klein reduction --- is a separate question, shown \emph{not} to hold for
the product construction used in this paper (\S\ref{sec:open}, ``full-operator
zero-mode gap''). By identifying the
