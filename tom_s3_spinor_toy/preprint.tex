...
The per-channel indices~\eqref{eq:l3partial} are proved; channel independence
(distinguishing the three $G_2$-equivariant bundles at the $G_2$ rather than $\mathrm{SU}(3)$
level) is the remaining open component of L3.
...
(i)~linear independence of the three zero modes / non-isomorphism of the three
$G_2$-equivariant bundles at the $G_2$ level (L3b, open; \S\ref{sec:open}).
...
\item \textbf{L3b --- Channel independence (\emph{open}).}
  The three $S^-$-subbundles from $\mathbf{8}_v, \mathbf{8}_s, \mathbf{8}_c$ are
  \emph{identical} as $\mathrm{SU}(3)$-modules $(1,0)\oplus(0,0)$. Proving that they
  are non-isomorphic as $G_2$-equivariant bundles over $S^6$ requires a $G_2$-level
  construction --- for example, three line bundles $\rho_0,\rho_1,\rho_2$ associated
  to the three 1-dimensional $\ZZ_3$-characters (noting $H^2(S^6)=0$ so $c_1=0$ for all
  line bundles; the distinction must arise at the $G_2$-representation level rather than
  topologically). An explicit equivariant bundle construction is needed to close this gap;
  the authors plan to address this in collaboration with T.~Lawrence.
