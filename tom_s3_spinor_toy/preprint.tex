...
\begin{lemma}[Geometric chirality, L5 --- unconditional net asymmetry; full statement conditional on L4B]\label{lem:chirality}
  ...
  Under the working L4B rank hypothesis ($\dim\ker(D^+|_{\mathbf 1})=1$, \S\ref{sec:kernel},
  open), all three zero modes are purely left-handed; if instead rank$=0$, two modes are
  left-handed and one right-handed, with the same net asymmetry. Either way, the chirality
...
\item \textbf{L4B --- $G_2$-equivariant character computation (\emph{partially resolved; rank
  ...
  confirmation; non-trivial-sector vanishing for the Levi-Civita operator directly
  ...
  torsion correction directly verified for $\rho=\mathbf{7},\mathbf{14}$) and experiments
  \texttt{20260713-round52-growing-gap-premise-audit} through
