"""Tests for W≈4 targeted product-discretized diagnostic."""

from __future__ import annotations

import json
import os
import subprocess
import sys
from pathlib import Path

from cc_toy_lab.spectral.s2_s1_product_discretized_w4_diagnostic import (
    build_w4_diagnostic_config,
    estimate_w4_case_count,
    run_w4_diagnostic,
    save_w4_diagnostic_artifacts,
)


def test_smoke_config_has_w_sweep() -> None:
    cfg = build_w4_diagnostic_config(smoke=True)
    assert cfg.w_values == (2.0, 4.0, 6.0, 8.0)


def test_smoke_case_count() -> None:
    cfg = build_w4_diagnostic_config(smoke=True)
    assert estimate_w4_case_count(cfg) == 32


def test_full_config_case_count() -> None:
    cfg = build_w4_diagnostic_config(smoke=False)
    assert estimate_w4_case_count(cfg) == 3600


def test_smoke_run_completes() -> None:
    cfg = build_w4_diagnostic_config(smoke=True)
    run = run_w4_diagnostic(cfg)
    assert len(run.cases) == 32
    assert run.classification


def test_artifacts_and_summary_non_claims(tmp_path: Path) -> None:
    cfg = build_w4_diagnostic_config(smoke=True)
    run = run_w4_diagnostic(cfg)
    paths = save_w4_diagnostic_artifacts(run, tmp_path / "w4")
    assert paths["config"].exists()
    assert paths["metrics"].exists()
    assert paths["data"].exists()
    assert paths["summary"].exists()
    assert (tmp_path / "w4" / "figures" / ".placeholder").exists()
    cfg_j = json.loads(paths["config"].read_text(encoding="utf-8"))
    assert cfg_j["w_values"] == [2.0, 4.0, 6.0, 8.0]
    text = paths["summary"].read_text(encoding="utf-8")
    assert "Scientific non-claims" in text
    assert "Witten" in text or "Lichnerowicz" in text
    blob = text + paths["metrics"].read_text(encoding="utf-8")
    assert "global_chiral_index" not in blob.lower()
    assert "physical_chirality_proven" not in blob.lower()


def test_metrics_row_has_required_fields(tmp_path: Path) -> None:
    cfg = build_w4_diagnostic_config(smoke=True)
    run = run_w4_diagnostic(cfg)
    save_w4_diagnostic_artifacts(run, tmp_path / "m")
    m = json.loads((tmp_path / "m" / "metrics.json").read_text(encoding="utf-8"))
    row = m["cases"][0]
    for k in (
        "localization_gate_v3_classification",
        "pass_rate_across_windows",
        "window_sensitivity_score",
        "unstable_window_cases",
        "window_robust_localization_passed",
        "localization_gate_v2_passed",
        "kernel_only_localization_gate_passed",
        "v2_vs_v3_disagreement",
        "clean_kernel_count",
        "disordered_kernel_count",
        "clean_min_abs_eigenvalue",
        "disordered_min_abs_eigenvalue",
    ):
        assert k in row


def test_cli_smoke(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    runs = tmp_path / "RUNS"
    env = {**os.environ, "CC_TOY_LAB_RUNS_ROOT": str(runs), "CC_TOY_LAB_FIXED_TIMESTAMP": "20990103-000042"}
    r = subprocess.run(
        [sys.executable, "scripts/s2_s1_product_discretized_w4_diagnostic.py"],
        cwd=root,
        capture_output=True,
        text=True,
        env=env,
        check=False,
    )
    assert r.returncode == 0, r.stderr + r.stdout
    rd = runs / "20990103-000042_s2_s1_product_discretized_w4_diagnostic"
    assert rd.exists()
    assert "w4_diagnostic_classification=" in r.stdout
