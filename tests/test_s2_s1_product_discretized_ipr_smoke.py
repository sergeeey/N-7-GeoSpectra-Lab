"""Tests for S2 x S1 Product-Discretized IPR Smoke"""
...
def test_ipr_smoke_script_exists():
    ...
    script_path = Path("scripts/s2_s1_product_discretized_ipr_smoke.py")
...
def test_ipr_clean_vs_disordered_mini():
    """Mini test: clean vs disordered IPR contrast."""
    q = 1
    s1_size = 8
    alpha = 0.0
    cutoff = 2
    ...
    # Clean case
    D_clean, _, _ = build_s2_s1_product_operator(
        q=q,
    ...
    eigvals_clean, eigvecs_clean = np.linalg.eigh(D_clean)
    ...
    # Disordered case
    D_dis, _, _ = build_s2_s1_product_operator(
        q=q,
        s1_family="spectral_circle",
        s1_size=s1_size,
        alpha=alpha,
    ...
    eigvals_dis, eigvecs_dis = np.linalg.eigh(D_dis)
...
def test_ipr_ratio_to_baseline():
    """Test IPR ratio to 1/N baseline."""
    q = 1
    s1_size = 16
...
def test_ipr_smoke_run_mini():
    """Mini run of IPR smoke (single family, small grid)."""
    from scripts.s2_s1_product_discretized_ipr_smoke import run_ipr_smoke
    ...
    output_dir = Path("reports/RUNS/test_ipr_smoke_mini")
